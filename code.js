...
(function () {
  ...
  function render() {
    ...
    else if (activeTab === 'wins') renderWins();
  ...
  function renderHistory() {
    ...
    if (allKeys.length === 0) {
      $('history-list').innerHTML = emptyStateHTML('No data yet');
      return;
    }
    ...
    $('history-list').innerHTML = allKeys.map(key => {
      const events   = DB.forDate(key).reverse();
      const used     = filterUsed(events);
      const resisted = filterByType(events, 'resisted');
      const habits   = getHabits(events);
      const totalAmt = sumAmount(used);

      const summary = buildHistorySummary(used, resisted);

      const cannabisRow = isCannabis
        ? historyStatRow('THC / CBD', `<span style="color:var(--thc)">${filterTHC(used).length}</span> / <span style="color:var(--cbd)">${filterCBD(used).length}</span>`)
        : '';

      return `<li class="history-day" data-key="${key}">
        <div class="history-day-header" onclick="App.toggleHistoryDay(this)">
          <span class="hd-date">${friendlyDate(key)}</span>
          <span class="hd-summary">${summary}</span>
        </div>
        <div class="history-day-detail">
          ${historyStatRow('Sessions', `${used.length} (${totalAmt} ${profile.amountUnit})`)}
          ${cannabisRow}
          ${historyStatRow('Resisted', resisted.length)}
          ${historyStatRow('Habits', habits.length)}
          <div class="hd-events">${events.map(e => eventRowHTML(e, true)).join('')}</div>
        </div>
      </li>`;
    }).join('');
  }
  ...
  function switchTab(tabName) {
    ...
    else if (tabName === 'wins') renderWins();
    else if (tabName === 'graph') requestAnimationFrame(renderGraphs);
  }
